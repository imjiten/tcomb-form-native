  ...
  getValue() {
    ...
    for (let i = 0, len = this.state.value.length; i < len; i++) {
      if (this.myRefs.hasOwnProperty(i)) {
        value.push(this.myRefs[i].getValue());
      }
    }
    return this.getTransformer().parse(value);
  }
...
class Form extends React.Component {
  ...
  pureValidate() {
    return this.myRef.input.pureValidate();
  }

  validate() {
    return this.myRef.input.validate();
  }
  ...
  getComponent(path) {
    ...
    return path.reduce((input, name) => input.refs[name], this.myRef.input);
  }
