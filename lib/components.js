...
class Struct extends Component {
  ...
  isValueNully() {
    return Object.keys(this.myRefs).every(ref => this.myRefs[ref].isValueNully());
  }

  removeErrors() {
    this.setState({ hasError: false });
    Object.keys(this.myRefs).forEach(ref => this.myRefs[ref].removeErrors());
  }

  getValue() {
    const value = {};
    for (const ref in this.myRefs) {
      value[ref] = this.myRefs[ref].getValue();
    }
  ...
  validate() {
    ...
    for (const ref in this.myRefs) {
      if (this.myRefs.hasOwnProperty(ref)) {
        result = this.myRefs[ref].validate();
...
export class List extends Component {
  ...
  removeErrors() {
    this.setState({ hasError: false });
    Object.keys(this.myRefs).forEach(ref => this.myRefs[ref].current.removeErrors());
  }

  getValue() {
    const value = [];
    for (let i = 0, len = this.state.value.length; i < len; i++) {
      if (this.myRefs.hasOwnProperty(i)) {
        value.push(this.myRefs[i].current.getValue());
      }
    }
    return this.getTransformer().parse(value);
  }
  ...
  validate() {
    ...
    for (let i = 0, len = this.state.value.length; i < len; i++) {
      result = this.myRefs[i].current.validate();
      errors = errors.concat(result.errors);
...
class Form extends React.Component {
  ...
  pureValidate() {
    return this.myRef.current.input.pureValidate();
  }

  validate() {
    return this.myRef.current.input.validate();
  }
  ...
  getComponent(path) {
    ...
    return path.reduce((input, name) => input.current[name], this.myRef);
  }
  ...
  render() {
    ...
    if (process.env.NODE_ENV !== "production") {
      t.assert(
        ...
        // console.log("OK", this.props.type)
